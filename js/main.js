...
const CONFIG = {
    ...
    colors: {
        ...
        burnt: new THREE.Color('#0a0200'), // Carvão quase preto
        ember: new THREE.Color('#ff3300'), // Brilho da brasa
        celebration: new THREE.Color('#f4a261'),
        future: new THREE.Color('#a8dadc'),
        clay: new THREE.Color('#8b4513'),
        storm: new THREE.Color('#1a1a2e'),
        smoke: new THREE.Color('#24110c') // NOVA COR: Fumaça densa
    },
...
// Fog começa preto, mas vamos animar a cor dele depois
...
// --- A. Partículas Principais (Chão com Brilho de Brasa) ---
...
const material = new THREE.ShaderMaterial({
    uniforms: {
        ...
        uEmberColor: { value: CONFIG.colors.ember },
    ...
    vertexShader: `
        ...
        void main() {
            ...
            vHeat = aRandom * (0.5 + 0.5 * sin(time * 3.0 + aRandom * 10.0));
        }
    `,
    fragmentShader: `
        uniform vec3 uColor;
        ...
        void main() {
            ...
            glow = pow(glow, 1.5);
            vec3 finalColor = mix(uColor, uEmberColor, vHeat * uHeatIntensity);
...
// --- D. Cinzas (Ashes - "Neve" de Fuligem) ---
...
const ashMaterial = new THREE.ShaderMaterial({
    uniforms: {
        uTime: { value: 0 },
        uColor: { value: new THREE.Color(0xcccccc) }, // Cinza claro
        uOpacity: { value: 0.0 }
    },
    vertexShader: `
        uniform float uTime; attribute vec3 aRandom; varying float vAlpha;
        void main() {
            vec3 pos = position;
            float time = uTime * 0.3; // Movimento lento e "pesado" do ar
            
            // Flutuação térmica: O calor empurra as cinzas um pouco para cima
            float upwardForce = sin(time + aRandom.z * 10.0) * 0.5; 
            float fallSpeed = 0.1 + aRandom.x * 0.2;
            
            pos.y -= (time * fallSpeed) - upwardForce * 0.05; 
            pos.y = mod(pos.y + 5.0, 20.0) - 5.0;
            
            // Espiral suave (vórtices de calor)
            pos.x += sin(time + aRandom.y * 10.0) * 0.3;
            pos.z += cos(time * 0.8 + aRandom.z * 10.0) * 0.3;
            
            vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
            gl_Position = projectionMatrix * mvPosition;
            gl_PointSize = (2.0 + aRandom.x * 4.0) * (10.0 / -mvPosition.z);
            vAlpha = 0.5 + 0.5 * sin(time + aRandom.x * 20.0); // Cintilação
        }
    `,
    fragmentShader: `
        uniform vec3 uColor; uniform float uOpacity; varying float vAlpha;
        void main() {
            float r = distance(gl_PointCoord, vec2(0.5)); if (r > 0.5) discard;
            gl_FragColor = vec4(uColor, vAlpha * 0.6 * uOpacity);
        }
    `,
...
const potteryPoints = []; for (let i = 0; i < 10; i++) { potteryPoints.push(new THREE.Vector2(Math.sin(i * 0.2) * 1 + 0.5, (i - 5) * 0.5)); }
...
artifactGroup.add(pottery); pottery.rotation.x = Math.PI / 6;

// --- Chuva ---
const rainTotal = CONFIG.rainCount; 
const rainGeo = new THREE.BufferGeometry();
const rainPos = new Float32Array(rainTotal * 2 * 3); 
const rainVel = new Float32Array(rainTotal);
for(let i=0; i<rainTotal; i++) {
    const x = (Math.random() - 0.5) * 40; const y = Math.random() * 30 + 10; const z = (Math.random() - 0.5) * 20;
...
stormSkyPlane.position.z = -20; scene.add(stormSkyPlane);
...
lightningBolt.visible = false; scene.add(lightningBolt);

function regenerateLightningPath(geo) {
     const posAttribute = geo.attributes.position; const array = posAttribute.array;
     let x = (Math.random() - 0.5) * 30; let y = 25; let z = (Math.random() - 0.5) * 15 - 5; const stepY = y / lightningSegmentCount;
     array[0] = x; array[1] = y; array[2] = z;
     for (let i = 1; i <= lightningSegmentCount; i++) { x += (Math.random() - 0.5) * 3; y -= stepY; z += (Math.random() - 0.5) * 2; array[i*3] = x; array[i*3+1] = y; array[i*3+2] = z; }
...
const featherCount = 20; const radius = 2; const featherGeometry = new THREE.ConeGeometry(0.1, 1.5, 8);
const featherMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.8, wireframe: true });
for (let i = 0; i < featherCount; i++) { const angle = (i / (featherCount - 1)) * Math.PI - Math.PI / 2; const feather = new THREE.Mesh(featherGeometry, featherMaterial); feather.position.x = Math.cos(angle) * radius; feather.position.y = Math.sin(angle) * radius; feather.position.z = 0; feather.rotation.z = angle - Math.PI / 2; cocarGroup.add(feather); }
cocarGroup.position.set(0, 1, -10); cocarGroup.visible = false;

// --- Floresta ---
const forestGroup = new THREE.Group();
scene.add(forestGroup); forestGroup.visible = false;
const treeGeometries = []; for (let i = 0; i < 50; i++) { const height = 1 + Math.random() * 2; const radius = 0.2 + Math.random() * 0.3; const treeGeo = new THREE.ConeGeometry(radius, height, 4, 1, true); const x = (Math.random() - 0.5) * 20; const z = (Math.random() - 0.5) * 10 - 5; treeGeo.translate(x, height / 2, z); treeGeometries.push(treeGeo); }
...
const sparkOffsets = new Float32Array(sparkTotal); // Para o movimento lateral
for(let i=0; i<sparkTotal; i++) { 
    ...
    sparkOffsets[i] = Math.random() * 100; // Offset aleatório para o seno
}
...
scene.add(constellationGroup); constellationGroup.visible = false;
const starTotal = 200; const starGeo = new THREE.BufferGeometry(); const starPos = new Float32Array(starTotal * 3);
for(let i=0; i<starTotal; i++) { starPos[i*3] = (Math.random() - 0.5) * 50; starPos[i*3+1] = (Math.random() - 0.5) * 30 + 10; starPos[i*3+2] = (Math.random() - 0.5) * 20 - 10; }
starGeo.setAttribute('position', new THREE.BufferAttribute(starPos, 3));
const starMaterial = new THREE.PointsMaterial({ color: 0xffffff, size: 0.15, transparent: true });
const stars = new THREE.Points(starGeo, starMaterial); constellationGroup.add(stars);
...
function animate() {
    ...
    // BRASAS TURBULENTAS (Melhoria)
    if (typeof sparks !== 'undefined' && sparks.visible) {
        const positions = sparks.geometry.attributes.position.array;
        for(let i=0; i<sparkTotal; i++) { 
            // Sobe
            positions[i*3+1] += sparkSpeed[i]; 
            // Turbulência lateral (Seno/Cosseno baseados no tempo + offset único)
            positions[i*3] += Math.sin(elapsedTime * 2.0 + sparkOffsets[i]) * 0.02; 
            positions[i*3+2] += Math.cos(elapsedTime * 1.5 + sparkOffsets[i]) * 0.02;
...
function scrambleText(element, text, duration) {
    const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()"; const originalText = text || element.innerText; const length = originalText.length; let progress = 0;
    const interval = setInterval(() => { progress += 1 / (duration * 30); if (progress >= 1) { element.innerText = originalText; clearInterval(interval); return; } let result = ""; for (let i = 0; i < length; i++) { if (i < length * progress) result += originalText[i]; else result += chars[Math.floor(Math.random() * chars.length)]; } element.innerText = result; }, 33);
}
const titles = document.querySelectorAll('.chapter-title');
titles.forEach(title => { title.dataset.originalText = title.innerText; ScrollTrigger.create({ trigger: title, start: "top 80%", onEnter: () => { gsap.fromTo(title, { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 1 }); scrambleText(title, title.dataset.originalText, 1.5); } }); });
const textBlocks = document.querySelectorAll('.text-block');
textBlocks.forEach(block => { gsap.to(block, { opacity: 1, y: 0, duration: 1, scrollTrigger: { trigger: block, start: "top 85%", end: "top 65%", scrub: true, toggleActions: "play none none reverse" } }); });

const knowledgeSection = document.querySelector('.knowledge-section'); const knowledgeContainer = document.querySelector('.knowledge-container');
if (knowledgeSection && knowledgeContainer) { gsap.to(knowledgeContainer, { x: () => -(knowledgeContainer.scrollWidth - window.innerWidth), ease: "none", scrollTrigger: { trigger: knowledgeSection, pin: true, scrub: 1, start: "top top", end: () => "+=" + (knowledgeContainer.scrollWidth - window.innerWidth), invalidateOnRefresh: true, onEnter: () => { cocarGroup.visible = true; gsap.to(cocarGroup.position, { z: 0, duration: 1 }); }, onLeave: () => { gsap.to(cocarGroup.position, { z: -10, duration: 1, onComplete: () => cocarGroup.visible = false }); }, onEnterBack: () => { cocarGroup.visible = true; gsap.to(cocarGroup.position, { z: 0, duration: 1 }); }, onLeaveBack: () => { gsap.to(cocarGroup.position, { z: -10, duration: 1, onComplete: () => cocarGroup.visible = false }); } } }); }
...
// Cinzas e Brasas (Aprimorado com Fumaça Colorida)
tl.call(() => { 
    isStormActive = false; rain.visible = false; lightningBolt.visible = false; scene.fog.color.setHex(0x050505); 
}, null, "step_fire")
...
.to(material.uniforms.uHeatIntensity, { value: 1.0, duration: 1, ease: smoothEase }, "step_fire") 
...
ScrollTrigger.create({ 
    ...
    onEnter: () => { 
        sparks.visible = true; 
        gsap.to(scene.fog, { density: 0.09, duration: 1 }); // Neblina densa
        gsap.to(scene.fog.color, { r: CONFIG.colors.smoke.r, g: CONFIG.colors.smoke.g, b: CONFIG.colors.smoke.b, duration: 1 }); // Cor de fumaça
    }, 
    onLeave: () => { 
        ...
        gsap.to(scene.fog.color, { r: 0.02, g: 0.02, b: 0.02, duration: 1 }); // Volta ao preto
    }, 
    onEnterBack: () => { 
        sparks.visible = true; 
...
tl.to(material.uniforms.uColor.value, { r: 0.2, g: 0.1, b: 0.05, duration: 1, ease: smoothEase }, "step_heroes").to(material.uniforms.uFormFactor, { value: 1.0, duration: 1, ease: smoothEase }, "step_heroes").to(camera.position, { z: 4, y: 1, duration: 1, ease: smoothEase }, "step_heroes");
...
window.addEventListener('load', () => {
    ...
    if (preloader) {
        window.scrollTo(0, 0);
        const hidePreloader = () => { gsap.to(preloader, { opacity: 0, duration: 1, onComplete: () => { preloader.style.visibility = 'hidden'; ScrollTrigger.refresh(); } }); };
        setTimeout(hidePreloader, 500);
        setTimeout(() => { if(preloader.style.visibility !== 'hidden') hidePreloader(); }, 4000);
    }
});